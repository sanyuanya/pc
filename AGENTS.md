...
常用命令模板
    ...
    # 📦 依赖管理（安装/同步依赖）
    # ================================
    # 根据 pyproject.toml + uv.lock 安装依赖到 .venv
    uv sync

    # 若修改了依赖，重新锁定（更新 uv.lock）
    uv lock

    # 强制同步（和 CI 一致，可复现构建）
    uv sync --frozen

    # 移除未使用依赖（类似 go mod tidy）
    uv pip prune
    ...
    # 🔍 静态检查 / Lint（可选）
    # ================================
    # ruff Lint（类似 go vet / golangci-lint）
    uv run ruff check .
    ...
    # 📦 生产构建（可选：打包为独立可执行文件）
    # ================================
    # 使用 uv build 构建 Python 项目
    #（需在 pyproject.toml 定义 [project]）
    uv build

    # 若需要单文件可执行（使用 PyInstaller）
